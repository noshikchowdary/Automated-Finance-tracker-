...
with st.sidebar.expander("Add Keyword to Category"):
    selected_cat = st.selectbox(
        "Select Category", list(CATEGORY_KEYWORDS.keys()), key="cat_select"
    )
...
if uploaded_file:
    try:
        if (
            hasattr(uploaded_file, "getbuffer")
            and uploaded_file.getbuffer().nbytes == 0
        ):
            ...
            try:
                ...
                st.error(
                    "The uploaded CSV file is empty. Please select a file with data."
                )
                df = None
            ...
            if df is not None:
                ...
                if missing_cols:
                    ...
                    if not expense_df.empty:
                        ...
                        expense_df["Month"] = (
                            expense_df["Date"].dt.to_period("M").astype(str)
                        )
                        monthly_summary = (
                            expense_df.groupby("Month")["Amount"].sum().reset_index()
                        )
